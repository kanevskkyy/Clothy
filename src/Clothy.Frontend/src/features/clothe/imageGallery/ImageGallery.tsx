...
import type { IClothePhotos } from "../../../entities/catalogService/interfaces/clothe/IClothePhotos.ts";
...
const ImageGallery: React.FC<ImageGalleryProps> = ({ additionalPhotos, selectedColor }) => {
    const sortedPhotos = useMemo<IClothePhotos[]>(() => {
        const colorPhotos = additionalPhotos.filter(p => p.colorId === selectedColor.id);
        if (!colorPhotos.length) return [];

        const mainPhoto = colorPhotos.find(p => p.isMain);
        const rest = colorPhotos.filter(p => p.id !== mainPhoto?.id);
    ...
    const safeIndex = Math.min(activeIndex, sortedPhotos.length - 1);
    const activePhoto = sortedPhotos[safeIndex];
