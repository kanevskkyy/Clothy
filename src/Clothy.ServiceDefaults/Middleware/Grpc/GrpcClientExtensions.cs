...
namespace Clothy.ServiceDefaults.Middleware.Grpc
{
    public static class GrpcClientExtensions
    {
        public static IHttpClientBuilder AddConfiguredGrpcClient<TClient>(this IServiceCollection services, string serviceName) where TClient : class
        {
            ...
            string? explicitAddress = config?[$"GrpcClients:{serviceName}"];

            var builder = services.AddGrpcClient<TClient>(options =>
                {
                    options.Address = new Uri(explicitAddress ?? $"https://{serviceName}");
                })
            ...
            if (explicitAddress == null) builder.AddServiceDiscovery();
