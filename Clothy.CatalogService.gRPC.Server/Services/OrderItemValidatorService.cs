        ...
        public override async Task<ValidateOrderItemsResponse> ValidateOrderItems(ValidateOrderItemsRequest request, ServerCallContext context)
        {
            ...
            try
            {
                foreach (var item in request.Items)
                {
                    ...
                    try
                    {
                        if (!Guid.TryParse(item.ClotheId, out var clotheId) || await unitOfWork.ClotheItems.GetByIdAsync(clotheId) == null)
                        {
                            isValid = false;
                            errorMessage = $"Clothe item not found with ID: {item.ClotheId}";
                            logger.LogWarning("Clothe item not found: {ClotheId}", item.ClotheId);
                        }
                        else if (!Guid.TryParse(item.ColorId, out var colorId) || await unitOfWork.Colors.GetByIdAsync(colorId) == null)
                        {
                            isValid = false;
                            errorMessage = $"Color not found with ID: {item.ColorId}";
                            logger.LogWarning("Color not found: {ColorId}", item.ColorId);
                        }
                        else if (!Guid.TryParse(item.SizeId, out var sizeId) || await unitOfWork.Sizes.GetByIdAsync(sizeId) == null)
                        {
                            isValid = false;
                            errorMessage = $"Size not found with ID: {item.SizeId}";
